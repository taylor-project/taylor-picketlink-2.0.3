...
public class SAML2AuthenticationHandler extends BaseSAML2Handler
{
   ...
   private class IDPAuthenticationHandler
   {
            ...
            throws ProcessingException
      {
         ...
         //Getting assertionConsumerService endpoint for the service provider configured in saml-entities.xml. 
         //Use it if destination is specified in the request. -CZ
         if ( destination == null ) {
	         String spEntityId = art.getIssuer().getValue();
	         destination = SAMLSPMetadata.getAssertionConsumerServiceEndpoint(spEntityId);
         }
         ...
         try
         {
            ...
            /* Let's invalidate the session here, we don't want to support single logout. -CZ  */
...
//            identityServer.stack().register(session.getId(), destination, isPost);
      ...
      public Document getResponse(SAML2HandlerRequest request) throws ConfigurationException, ProcessingException
      {
         ...
         //Always return unspecified format back. --CZ
         String returningNameIdFormat = JBossSAMLURIConstants.NAMEID_FORMAT_UNSPECIFIED.get();
         
         //Return the Service Provider requested NameIDFormat back. _CZ
//         if ( nameIdFormat != null ) {
//	         if ( nameIdFormat.equals(JBossSAMLURIConstants.NAMEID_FORMAT_EMAIL.get()) 
//	        		 || nameIdFormat.equals(JBossSAMLURIConstants.NAMEID_FORMAT_ENTITY.get())  
...
//	        	 returningNameIdFormat = nameIdFormat;
//	         }
//         }
         ...
         if ( request.getSAML2Object() instanceof AuthnRequestType ) {
        	 //Add Audience conditions to saml issuer in the request. -CZ
        	 AudienceRestrictionType audienceRestrictionType = new AudienceRestrictionType();
        	 try {
        		 String audience = ((AuthnRequestType)request.getSAML2Object()).getIssuer().getValue();
         ...
         //Don't return roles back, disable it.
         ...
         {
            AttributeStatementType attStatement = StatementUtil.createAttributeStatement(attribs, request.getOptions());
