...
public class IDPWebRequestUtil
{
   ...
   {
	   //No need to check trusted based on domain settings. Service providers are configured. -CZ
	   /*
   ...
   public void send(WebRequestUtilHolder holder) throws GeneralSecurityException, IOException
   {
      ...
      //EncryptedAttribute is added to response document as CDATA section because there is no easy way to 
      //write Element to XMLStreamWriter (we could traverse the element and add the xml to XMLStreamWriter 
      //but that is more complicated.), here is logic to remove <![CDATA[ and ]]> from EncryptedData
      //elements. -CZ
      String documentAsString = DocumentUtil.getDocumentAsString(responseDoc);
      documentAsString = documentAsString.replaceAll("<!\\[CDATA\\[(<xenc:EncryptedData.*?</xenc:EncryptedData>)\\]\\]>", "$1");
      responseDoc = DocumentUtil.getDocument(documentAsString);
