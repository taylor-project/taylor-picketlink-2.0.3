...
public class SAML2LogOutHandler extends BaseSAML2Handler
{
   ...
   private class IDPLogOutHandler
   {
            ...
            throws ProcessingException
      {
         ...
         //Getting singleLogoutServiceSP callback url for the service provider configured in saml-entities.xml.  -CZ
         String spEntityId = logOutRequest.getIssuer().getValue();
         String singleLogoutServiceSPUrl = SAMLSPMetadata.getSingleLogoutServiceEndpoint(spEntityId);
         ...
         try
         {
            ...
            //Participant is not necessarily the same as originalIssuer -CZ
            if (participant == null || singleLogoutServiceSPUrl != null )
            {
               //All log out is done
               session.invalidate();
               ...
               //Use singleLogoutServiceSPUrl configured in saml-entities as participant if it is there. -CZ
               generateSuccessStatusResponseType(logOutRequest.getID(), request, response, singleLogoutServiceSPUrl != null ? singleLogoutServiceSPUrl : participant);
               response.setPostBindingForResponse(isPost.booleanValue());
      ...
      private void generateSuccessStatusResponseType(String logOutRequestID, SAML2HandlerRequest request,
            SAML2HandlerResponse response, String participant) throws ConfigurationException,
            ParserConfigurationException, ProcessingException
      {
         if (trace)
         {
            log.trace("Generating Success Status Response for " + participant);
         }
...
//         statusCodeType.setStatusCode(status2ndLevel);
         ...
         //Above logic is not correct, STATUS_RESPONDER means an error on the part of the SAML responder. 
         //To return success logout back, we need to set top level status code to STATUS_SUCCESS. -CZ
         StatusType statusType = new StatusType();;
         StatusCodeType statusCodeType = new StatusCodeType();
   ...
   private class SPLogOutHandler
   {
            ...
            throws ProcessingException
      {
         ...
         if (statusCode.getValue().toString().equals(JBossSAMLURIConstants.STATUS_SUCCESS.get()) || secondLevelstatusCode.getValue().toString().equals(JBossSAMLURIConstants.STATUS_SUCCESS.get()))
         {
