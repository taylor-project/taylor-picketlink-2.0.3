...
import org.picketlink.identity.federation.web.util.SAMLSPMetadata;
...
public class IDPWebBrowserSSOValve extends ValveBase implements Lifecycle {
	...
	/**
	 * SAML Web Browser SSO Profile has a requirement that the IDP does not
	 * respond back in Redirect Binding. Set this to true if you want the IDP to
	 * adhere to this requirement via
	 */
	...
	public void invoke(Request request, Response response) throws IOException,
		...
		//CZ
		boolean idpInitiatedLogin = isUnsolicitedRequest(request);
		
		String idpInitiatedLoginServiceProviderId = request
				.getParameter("spid");
		String idpInitiatedLoginTargetUrl = request.getParameter("target");
		...
		if (containsSAMLRequestMessage || containsSAMLResponseMessage) {
			...
			if (isNotNull(samlRequestMessage)) {
				session.setNote(GeneralConstants.SAML_REQUEST_KEY,
						samlRequestMessage);
				// Parsing SAMLRequestMessage and set the parsed object in the
				// session. -CZ
				try {
					IDPWebRequestUtil webRequestUtil = new IDPWebRequestUtil(
							request, idpConfiguration, keyManager);
					...
					request.getSession().setAttribute(
							"ServiceProvider",
							((RequestAbstractType) samlObject).getIssuer()
									.getValue());
				} catch (Exception e) {
					throw new RuntimeException(
							"Received an invalid SAML Request.", e);
				}
			}
		...
		} else if (idpInitiatedLogin) {
			// To support IdP initiated login. -CZ
			if ( idpInitiatedLoginTargetUrl != null ) {
				session.setNote(GeneralConstants.RELAY_STATE,
					idpInitiatedLoginTargetUrl);
			}
			request.getSession().setAttribute("ServiceProvider",
					idpInitiatedLoginServiceProviderId);
		...
		if (userPrincipal == null) {
			try {
				...
				if (trace)
					log.trace("Referer in finally block=" + referer
		...
		if (response.getStatus() == HttpServletResponse.SC_FORBIDDEN) {
			try {
				...
				holder.setResponseDoc(samlErrorResponse)
						.setDestination(referer).setRelayState(relayState)
						.setAreWeSendingRequest(false).setPrivateKey(null)
		...
		if (userPrincipal != null) {
			 ...
			 * from the SP
			 */
			samlRequestMessage = (String) session
					.getNote(GeneralConstants.SAML_REQUEST_KEY);

			samlResponseMessage = (String) session
			...
			if (samlRequestMessage != null) {
				...
				// TODO: Get the target from service provider metadata? Charles
				// Zhao
				String target = request.getParameter(SAML11Constants.TARGET);
				if (isNotNull(target)) {
					// We have SAML 1.1 IDP first scenario. Now we need to
					// create a SAMLResponse and send back
					// to SP as per target
					handleSAML11(webRequestUtil, request, response);
				} else if (idpInitiatedLogin) {
					// Handle SAML 2.0 IDP first scenario. -CZ
					handleSAML20Unsolicited(webRequestUtil, request, response,
							idpInitiatedLoginTargetUrl,
							idpInitiatedLoginServiceProviderId);
				} else {
					if (trace)
						log.trace("SAML 1.1::Proceeding to IDP index page");
					RequestDispatcher dispatch = context.getServletContext()
							.getRequestDispatcher("/hosted/");
					try {
	...
	// Support IdP initiated login. -CZ
	private boolean isUnsolicitedRequest(Request request) {
		try {
			return request.getRequestURL().toString().endsWith("/unsolicited");
		} catch (Exception e) {
			return false;
		}
	}

	// TODO Refactor to return SAML 2.0 response back.
	// Handles IdP first login scenario with SAML 2.0 unsolicited response. -CZ
	protected void handleSAML20Unsolicited(IDPWebRequestUtil webRequestUtil,
			Request request, Response response, String target,
			String idpInitiatedLoginServiceProviderId) throws ServletException,
			IOException {
		try {
			String assertionConsumerEndpoint = SAMLSPMetadata
					.getAssertionConsumerServiceEndpoint(idpInitiatedLoginServiceProviderId);
			
			Document samlResponse = getUnsolicitedResponse(request, idpInitiatedLoginServiceProviderId);

			WebRequestUtilHolder holder = webRequestUtil.getHolder();

			holder.setResponseDoc(samlResponse)
					.setDestination(assertionConsumerEndpoint)
					.setRelayState(target).setAreWeSendingRequest(false)
					.setPrivateKey(null).setSupportSignature(false)
					.setServletResponse(response).setPostBindingRequested(true);
	...
	private Document getUnsolicitedResponse(Request request, String idpInitiatedLoginServiceProviderId) throws ConfigurationException, ProcessingException {
        ...
        //Use emailAddress name id to work with TFIM.
        String returningNameIdFormat = JBossSAMLURIConstants.NAMEID_FORMAT_UNSPECIFIED.get();
        ...
        idp.setNameIDFormat(returningNameIdFormat);
        
        SPInfoHolder sp = new SPInfoHolder();
        
        sp.setResponseDestinationURI(SAMLSPMetadata.getAssertionConsumerServiceEndpoint(idpInitiatedLoginServiceProviderId));
        
        ResponseType responseType = saml2Response.createResponseType(id, sp, idp, issuerHolder);
        ...
        AssertionType assertion = responseType.getAssertions().get(0).getAssertion();
       	 ...
       	 try {
       		 URI uri = new URI(idpInitiatedLoginServiceProviderId);
       		 audienceRestrictionType.addAudience(uri );
       ...
       addRequestOptionsForAttributeStatement(options, idpInitiatedLoginServiceProviderId);
       
       AttributeStatementType attStatement = StatementUtil.createAttributeStatement(getSamlAttributes(userPrincipal, 0, idpInitiatedLoginServiceProviderId), options );
       assertion.addStatement(attStatement);

        //Add assertion to the session
        request.getSession().setAttribute(GeneralConstants.ASSERTION, assertion);
		...
		try
        {
			//Convert SAML Response object to SAMLResponse Document to be returned back. -CZ
	...
	protected void handleSAML11(IDPWebRequestUtil webRequestUtil,
			Request request, Response response) throws ServletException,
			IOException {
		try {
			Principal userPrincipal = request.getPrincipal();
			...
			Document samlResponse = DocumentUtil
	...
	protected void processSAMLRequestMessage(IDPWebRequestUtil webRequestUtil,
		...
		try {
			...
			String tokenSignatureValidatingAlias = getTokenSignatureValidatingAlias(
					request, issuer);
			...
			ProtocolContext protocolContext = new HTTPContext(request,
					response, context.getServletContext());
			...
			if (this.keyManager != null) {
				if (trace) {
					...
					log.trace("Validating Alias="
							+ tokenSignatureValidatingAlias);
				}

				PublicKey validatingKey = CoreConfigUtil.getValidatingKey(
						keyManager, tokenSignatureValidatingAlias);
			...
			// Use the public key in saml-entities for the service provider if
			// configured. Also preparing secret key for EncryptedAttributes. -CZ
			String spEntityId = null;
			if (samlObject instanceof RequestAbstractType) {
				spEntityId = ((RequestAbstractType) samlObject).getIssuer()
						.getValue();
				addRequestOptionsForAttributeStatement(requestOptions, spEntityId);
			}

			// Propagate NameIDPolicy in the AuthnRequest. -CZ
			if (samlObject instanceof AuthnRequestType) {
				NameIDPolicyType nameIDPolicy = ((AuthnRequestType) samlObject)
						.getNameIDPolicy();
				if (nameIDPolicy != null) {
					String nameIdFormat = nameIDPolicy.getFormat().toString();
					requestOptions.put(GeneralConstants.NAMEID_FORMAT,
							nameIdFormat);
				}
			}

			Integer attributeConsumingServiceIndex = null;
			if ( saml2HandlerRequest instanceof AuthnRequestType ) {
				AuthnRequestType authnRequest = (AuthnRequestType) saml2HandlerRequest
						.getSAML2Object();
				attributeConsumingServiceIndex = authnRequest
						.getAttributeConsumingServiceIndex();
			}
			
			Map<String, Object> attribs = getSamlAttributes(userPrincipal,
					attributeConsumingServiceIndex, spEntityId);
			...
			try {
				boolean postProfile = webRequestUtil
						.hasSAMLRequestInPostProfile();
				if (postProfile)
					recycle(response);

				WebRequestUtilHolder holder = webRequestUtil.getHolder();
				holder.setResponseDoc(samlResponse).setDestination(destination)
	...
	private void addRequestOptionsForAttributeStatement(
			Map<String, Object> requestOptions, String spEntityId)
			throws TrustKeyConfigurationException, TrustKeyProcessingException {
		X509Certificate signingCert = SAMLSPMetadata
				.getSigningCert(spEntityId);
		if (signingCert != null) {
			requestOptions.put(GeneralConstants.SENDER_PUBLIC_KEY,
					signingCert.getPublicKey());
		}
		X509Certificate encryptionCert = SAMLSPMetadata
				.getEncryptionCert(spEntityId);
		if (encryptionCert != null) {
			requestOptions.put(
					GeneralConstants.SENDER_ENCRYPTION_PUBLIC_KEY,
					encryptionCert.getPublicKey());
		}
		// Encryption key size.
		if (this.keyManager != null) {
			int keySize = this.keyManager.getEncryptionKeySize();
			requestOptions.put(GeneralConstants.ENCRYPTING_SECRET_KEY,
					keyManager
							.getEncryptionKey(spEntityId,
									this.keyManager.getEncryptionAlg(),
									keySize));
			requestOptions.put(
					GeneralConstants.ENCRYPTING_SECRET_KEY_SIZE,
					keySize);
		}
	}

	private Map<String, Object> getSamlAttributes(Principal userPrincipal, Integer attributeConsumerServiceIndex, String spEntityId) {
		// Getting requested attributes from saml-entities.xml for the
		// service provider, and pass that to AttributeManager to retrieve the attribute values.
		// -CZ
		List<String> allAttributeKeys = new ArrayList<String>();
		allAttributeKeys.addAll(this.attributeKeys);

		allAttributeKeys.addAll(SAMLSPMetadata.getRequestedAttributes(
				spEntityId, attributeConsumerServiceIndex));
		
		Map<String, Object> attribs = this.attribManager.getAttributes(
				userPrincipal, allAttributeKeys);
		return attribs;
	}

	/**
	 * Returns the alias to be used for the token's signature verification. If
	 * <code>validatingAliasToTokenIssuer</code> is true the token issuer will
	 * be returned.
	 * 
	 * @param request
	 * @param issuer
	 * @return
	 */
	private String getTokenSignatureValidatingAlias(Request request,
			String issuer) {
	...
	protected void processSAMLResponseMessage(IDPWebRequestUtil webRequestUtil,
		...
		try {
			...
			String tokenValidatingAlias = getTokenSignatureValidatingAlias(
					request, issuer);
			...
			if (isPost) {
				...
				if (ignoreIncomingSignatures == false
						&& signOutgoingMessages == true) {
					PublicKey publicKey = keyManager
							.getValidatingKey(tokenValidatingAlias);
					isValid = samlSignature.validate(
							samlDocumentHolder.getSamlDocument(), publicKey);
				} else
					isValid = true;
			} else {
				isValid = validate(tokenValidatingAlias,
						request.getQueryString(), new SessionHolder(
								samlResponseMessage, signature, sigAlg), isPost);
			}
			...
			try {
				...
				WebRequestUtilHolder holder = webRequestUtil.getHolder();
				if (destination == null)
					throw new ServletException(ErrorCodes.NULL_VALUE
							+ "Destination");
				holder.setResponseDoc(samlResponse).setDestination(destination)
						.setRelayState(relayState)
						.setAreWeSendingRequest(willSendRequest)
	...
	protected void sendErrorResponseToSP(String referrer, Response response,
		...
		Document samlResponse = webRequestUtil.getErrorResponse(referrer,
				JBossSAMLURIConstants.STATUS_RESPONDER.get(), this.identityURL,
	...
	protected boolean validate(String remoteAddress, String queryString,
		...
		if (!this.ignoreIncomingSignatures && !isPost) {
			...
			byte[] sigValue = RedirectBindingSignatureUtil
			...
			// Post binding no signature verification. The SAML message
	...
	public void start() throws LifecycleException {
		...
		if (StringUtil.isNullOrEmpty(samlHandlerChainClass))
			...
			try {
				chain = SAML2HandlerChainFactory
						.createChain(this.samlHandlerChainClass);
		...
		// Loading SAML SP metadata from saml-entities.xml. -CZ
		try {
			log.info("Loading SAML Service Providers Metadata from "
					+ GeneralConstants.SAML_ENTITIES_FILE_LOCATION);
			SAMLSPMetadata.init(context.getServletContext()
					.getResourceAsStream(
							GeneralConstants.SAML_ENTITIES_FILE_LOCATION));
		} catch (ParsingException e) {
			log.warn("Unable to parse saml-entities.xml.", e);
		...
		if (idpConfiguration == null) {
			if (is == null)
				throw new RuntimeException(
						ErrorCodes.IDP_WEBBROWSER_VALVE_CONF_FILE_MISSING
								+ configFile);
		...
		try {
			...
			this.canonicalizationMethod = idpConfiguration
					.getCanonicalizationMethod();
			log.info("IDPWebBrowserSSOValve:: Setting the CanonicalizationMethod on XMLSignatureUtil::"
					+ canonicalizationMethod);
			XMLSignatureUtil
					.setCanonicalizationMethodType(canonicalizationMethod);
		...
		if (this.signOutgoingMessages) {
			...
			if (keyProvider == null)
				throw new LifecycleException(ErrorCodes.NULL_VALUE
						+ "Key Provider is null for context="
						+ context.getName());
		...
		try {
			...
			handlers = ConfigurationUtil.getHandlers(context
		...
		IdentityServer identityServer = (IdentityServer) context
		...
		if (identityServer == null) {
			...
			if (StringUtil.isNotNull(this.identityParticipantStack)) {
				try {
					Class<?> clazz = SecurityActions.loadClass(getClass(),
							this.identityParticipantStack);
					if (clazz == null)
						throw new ClassNotFoundException(
								ErrorCodes.CLASS_NOT_LOADED
										+ this.identityParticipantStack);
					...
					log.error(
							"Unable to set the Identity Participant Stack Class. Will just use the default",
							e);
				}
			}
		}
