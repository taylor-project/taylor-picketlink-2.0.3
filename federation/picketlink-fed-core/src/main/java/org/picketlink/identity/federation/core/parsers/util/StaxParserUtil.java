...
public class StaxParserUtil
{
   ...
   public static Validator getSchemaValidator() throws SAXException, IOException
   {
	...
	public static Object getElement(XMLEventReader xmlEventReader) throws ParsingException {
	      ByteArrayOutputStream baos = new ByteArrayOutputStream();
	      XMLStreamWriter writer;
		try {
			writer = StaxUtil.getXMLStreamWriter(baos);
			parseElement(writer, xmlEventReader);
			writer.flush();
	...
	public static void parseElement(XMLStreamWriter writer, XMLEventReader xmlEventReader) throws ParsingException, XMLStreamException {
		StartElement startElement = getNextStartElement(xmlEventReader);
		String startTag = getStartElementName(startElement);
		writer.writeStartElement(startTag);
		 Iterator iterator = startElement.getNamespaces();
        while (iterator.hasNext()) {
        	Namespace  attr = (Namespace ) iterator.next();
          writer.writeNamespace(attr.getPrefix(), attr.getNamespaceURI());
        }	
	      ...
	      while (xmlEventReader.hasNext())
	      {
	         ...
	         if (xmlEvent instanceof EndElement)
	         {
	            ...
	            if (StaxParserUtil.matches(end, startTag)) {
