...
public class StatementUtil
{
   ...
   {
	   return createAttributeStatement(attributes, new HashMap<String, Object>());
   }
   /**
    * Create an attribute statement with all the attributes
    * @param attributes a map with keys from {@link AttributeConstants}
    * @return
    */
   public static AttributeStatementType createAttributeStatement(Map<String, Object> attributes, Map<String, Object> options)
   {
      ...
      for (String key : keys)
      {
         ...
         else
         {
            ...
            //Add the logic to support encrypted attributes. -CZ
            if ( value instanceof AttributeHolder ) {
            	Object attributeValue = ((AttributeHolder)value).getAttributeValue();
            	addAttribute((AttributeHolder)value, attrStatement, key, attributeValue, options);
            } else {
   ...
   /**
	* Helper method to add attribute to AttributeStatement, support EncryptedAttribute.
	* 
	* @author Charles Zhao
   */
  private static void addAttribute(AttributeHolder attributeHolder, AttributeStatementType attrStatement, String key, Object attributeValue, Map<String, Object> options) {
	   AttributeType attributeType = new AttributeType(key);
	   attributeType.setNameFormat(JBossSAMLURIConstants.ATTRIBUTE_FORMAT_URI.get());
	   
	   if ( attributeValue instanceof Collection ) {
		   //Adding mulit-value attributes as multiple attribute values of the same
		   //attribute.
		   for ( Object attributeValueItem : (Collection)attributeValue ) {
			   attributeType.addAttributeValue(attributeValueItem);
		   }
	   } else {
		   attributeType.addAttributeValue(attributeValue);
	   }
	   
	   if ( attributeHolder.isEncryptedRequired() ) {
		   EncryptedElementType encryptedAttributeType = new EncryptedElementType();
		   
		   try {
			   Document attributeDocument = SAMLUtil.toSAMLAttributeDocument(attributeType);

			   PublicKey publicKey = (PublicKey) options.get("SENDER_ENCRYPTION_PUBLIC_KEY");
			   SecretKey secretKey = (SecretKey)options.get("ENCRYPTING_SECRET_KEY");
			   int keySize = (Integer) options.get("ENCRYPTING_SECRET_KEY_SIZE");
			   
			   XMLEncryptionUtil.encryptElement(attributeDocument, attributeDocument.getDocumentElement(), publicKey, secretKey, keySize);
			   
			   encryptedAttributeType.setEncryptedElement(attributeDocument.getDocumentElement());
		   } catch(Exception e) {
			   throw new RuntimeException("Unabled to create EncryptedAttribute for " + key, e);
		   }
		   
		   attrStatement.addAttribute(new ASTChoiceType(encryptedAttributeType));
	   } else {
		   attrStatement.addAttribute(new ASTChoiceType(attributeType));
	   }
