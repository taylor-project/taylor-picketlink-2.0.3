...
public class SAMLEntityDescriptorParser implements ParserNamespaceSupport
{
         ...
         StartElement startElement) throws ParsingException
   {
      ...
      //Fixing the issue where isDefault attribute is not parsed hence missing in the metadata. -CZ
      Attribute isDefaultAttr = startElement.getAttributeByName(new QName(JBossSAMLConstants.ISDEFAULT.get()));
      if ( isDefaultAttr == null ) {
    	  attributeConsumer.setIsDefault(false);
      } else {
    	  attributeConsumer.setIsDefault(Boolean.valueOf(StaxParserUtil.getAttributeValue(isDefaultAttr)));
      }
