...
public class BaseWriter
{
   ...
   /**
    * Write an {@code EncryptedElementType} to stream
    * 
    * @author Charles Zhao
    */
   public void write(EncryptedElementType encryptedElementType) throws ProcessingException
   {
	   StaxUtil.writeStartElement(writer, ASSERTION_PREFIX, JBossSAMLConstants.ENCRYPTED_ATTRIBUTE.get(), ASSERTION_NSURI.get());
	   
	   writeAttributeTypeWithoutRootTag(encryptedElementType);
    ...
    * @author Charles Zhao
    */
	private void writeAttributeTypeWithoutRootTag(EncryptedElementType encryptedElementType) {
		Element encryptedElement = encryptedElementType.getEncryptedElement();
		try {
			DocumentUtil.getNodeAsString(encryptedElement);
			writer.writeCData(DocumentUtil.getNodeAsString(encryptedElement));
		} catch (Exception e) {
			throw new RuntimeException("Unable to write EncryptedElement to SAML Response.", e);
		}
	}

/**
    * Write an {@code AttributeType} to stream with namespace definitions. This is to allow us to create an SAML attribute
    * Element for encryption.
    * 
    * @author Charles Zhao
    */
   public void writeAttributeTypeWithNS(AttributeType attributeType) throws ProcessingException {
      StaxUtil.writeStartElement(writer, ASSERTION_PREFIX, JBossSAMLConstants.ATTRIBUTE.get(), ASSERTION_NSURI.get());
      StaxUtil.writeNameSpace(writer, ASSERTION_PREFIX, ASSERTION_NSURI.get());

      writeAttributeTypeWithoutRootTag(attributeType, true);
   ...
   public void writeAttributeTypeWithoutRootTag(AttributeType attributeType) throws ProcessingException {
	   writeAttributeTypeWithoutRootTag(attributeType, false); 
   }

   public void writeAttributeTypeWithoutRootTag(AttributeType attributeType, boolean writeAttributeValueInCData) throws ProcessingException
   {
      ...
      if (attributeValues != null)
      {
         for (Object attributeValue : attributeValues)
         {
        	 if (attributeValue != null) {
	            ...
	            {
	               if ( writeAttributeValueInCData ) {
	            	   writeStringAttributeValueInCData((String)attributeValue);
	               } else {
	            	   writeStringAttributeValue((String) attributeValue);
	               }
	            }
	            //If attributeValue is XML Element instance, then write in CDATA section. 
	            //This logic also applies for attribute that is to be encrypted. -CZ
	            else if ( attributeValue instanceof Element ) {
	            	try {
	            		writeElementAttributeValueInXml((Element)attributeValue);
	            	} catch (Exception e) {
	            		throw new RuntimeException("Unable to write XML Element to SAML Response in CDATA section.", e);
	            	}
	            }
	            else 
   ...
   /**
    * Added this method to support writing attribute in CDATA section.
    * 
    * @author Charles Zhao
    */
   private void writeStringAttributeValueInCData(String attributeValue) throws ProcessingException
   {
	   ...
	   StaxUtil.writeCData(writer, attributeValue);
	   StaxUtil.writeEndElement(writer);
   }
 ...
 * @throws XMLStreamException 
    */
   private void writeElementAttributeValueInXml(Element attributeElement) throws ProcessingException, XMLStreamException
   {
	   StaxUtil.writeStartElement(writer, ASSERTION_PREFIX, JBossSAMLConstants.ATTRIBUTE_VALUE.get(),
			   ASSERTION_NSURI.get());
	   ...
	   StaxUtil.writeNameSpace(writer, JBossSAMLURIConstants.XSI_PREFIX.get(), JBossSAMLURIConstants.XSI_NSURI.get());
	   StaxUtil.writeNameSpace(writer, "xs", JBossSAMLURIConstants.XMLSCHEMA_NSURI.get());
	   StaxUtil.writeAttribute(writer, "xsi", JBossSAMLURIConstants.XSI_NSURI.get(), "type", "xs:anyType");
	   
	   writeElement(writer, attributeElement, true);
	   
	   StaxUtil.writeEndElement(writer);
   }
   
	private static void writeElement(XMLStreamWriter writer, Element element, boolean addNamespace) throws XMLStreamException {
		...
		for (int i = 0; i < childNodes.getLength(); i++  ) {
			...
			if ( item instanceof Element ) {
				writeElement(writer, ((Element)item), false);
			} else if ( item instanceof Text ) {
				writer.writeCharacters(((Text)item).getData());
			}
		}
